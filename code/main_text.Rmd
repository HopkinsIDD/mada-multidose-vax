...
```{r}
vcard <- read_csv("vaccard_final_KM.csv")
...
natcov<-read_csv("nad_data.csv", col_types = paste(rep("c",189),sep="", collapse="")) %>%
...
ftdf2 <- bind_rows(
        ...
        summarize(medianTTV=median(V1.SA.time),
                       lqTTV=quantile(V1.SA.time,0.25),
                       uqTTV=quantile(V1.SA.time,0.75)
                       ) %>%
        mutate(Dose="First") %>%
        rename(Type=vwStatus1),

        fullobs2 %>% group_by(vwStatus2)%>%
                summarize(medianTTV=median(V2.SA.time),
                               lqTTV=quantile(V2.SA.time,0.25),
                               uqTTV=quantile(V2.SA.time,0.75)
                               ) %>%
                mutate(Dose="Second")%>%
                rename(Type=vwStatus2),

        fullobs3 %>% group_by(vwStatus3)%>%
                summarize(medianTTV=median(V3.SA.time),
                               lqTTV=quantile(V3.SA.time,0.25),
                               uqTTV=quantile(V3.SA.time,0.75)
                               ) %>%
                mutate(Dose="Third")%>%
